import {OutcomesState} from "@/types/app.types";
import Headline from "@/components/Headline";
import {toFormatted} from "@/utils";
import {PUBLIC_DATE_FORMAT} from "@/constants";
import Card from "@/components/Card";

const Outcomes = ({ovulationDate, fertileWindow, nextPeriod, pregnancyTestDay, expectedDueDate}: OutcomesState) => {
  return (
      <>
        <Headline primary='Outcomes'
                  secondary='Start tracking your ovulation today and take control of your fertility journey.'/>
        <div className="mt-5 grid grid-cols-1 md:grid-cols-3 gap-6">
          <Card date={`${toFormatted(fertileWindow?.[0], PUBLIC_DATE_FORMAT)} - ${toFormatted(fertileWindow?.[1], PUBLIC_DATE_FORMAT)}`} title='Fertile Window'
          ...
          <Card date={toFormatted(ovulationDate, PUBLIC_DATE_FORMAT)} title='Ovulation Date'
          ...
          <Card date={toFormatted(nextPeriod, PUBLIC_DATE_FORMAT)} title='Next Period Date'
          ...
          <Card date={toFormatted(pregnancyTestDay, PUBLIC_DATE_FORMAT)} title='Pregnancy Test Date'
          ...
          <Card date={toFormatted(expectedDueDate, PUBLIC_DATE_FORMAT)} title='Expected Due Date'
